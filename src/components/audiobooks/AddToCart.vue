<template>
  ...
  export default {
    ...
    setup() {
       onMounted(()=>{
         db.collection("audiobooks").orderBy('timestamp')
      ...
      async function addProduct(product) {
        ...
        let p = {...product}
        console.log(p)
        await db.collection('audiobooks').doc(`${product.id}`).set(p,{merge:true})
        
      }
      async function removeProduct(product) {
        
        await db.collection('audiobooks').doc(`${product.id}`).delete()
         products.value.map((p) => {
          if (product.id == p.id) {
            p.cart = false;
          }
      ...
      function increaseQ(product) {
          ...
          db.collection('audiobooks').doc(`${product.id}`).update({
